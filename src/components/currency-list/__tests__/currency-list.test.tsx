...
import { CurrencyListCoponent } from "../currency-list.component";
import { TProps } from '../currency-list-item/currency-list-item.component';

jest.mock('src/components/currency-list/currency-list-item', () => ({ data, baseCurrency }: TProps) => (<div>CurrencyListItemComponent
	<div>{data.ticker}</div>
...
describe('CurrencyListComponent', () => {
	...
	it("Loads and renders list items", async () => {
		...
		const loader = screen.getByText('LOADING');
		expect(loader).toBeInTheDocument();
		const listElements = await screen.findAllByText('CurrencyListItemComponent');
